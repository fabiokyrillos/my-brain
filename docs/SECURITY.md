...
- O mesmo exercício revelou que levantar SQLSTATE `40001` de qualquer RPC neste projeto trava a requisição até o timeout do gateway — reproduzido inclusive na já publicada `correct_entry_interpretation` (Fase 2B) via chamada direta ao REST, sem nenhum código deste repositório envolvido. `confirm_entry_task_candidates` usa `55P03` em vez disso. O caminho equivalente de `correct_entry_interpretation` não foi alterado (fora do escopo desta slice) e fica registrado como pendência urgente em `DECISIONS.md`/`TODO.md`: hoje, um usuário real que dispare esse conflito de versão em produção tem a requisição travada por um minuto em vez de ver a mensagem de recarregar.
...
- **Urgente:** `correct_entry_interpretation` (Fase 2B) sinaliza conflito de versão com SQLSTATE `40001`, que trava a requisição até o timeout do gateway no projeto linkado em vez de retornar o erro — descoberto durante a Slice 2X.7, não corrigido nela (fora do escopo do arquivo). Ver `DECISIONS.md`.
