...
### Phase 2F — One Write Path (re-scoped; see `docs/initiatives/phase-2f/PHASE_2F_PRD.md`)

**Phase 2F is COMPLETE.** All six slices (2F.1–2F.6) are implemented, reviewed, accepted and merged, and the two that carried a migration are deployed. Slice 2F.6 merged as PR #33 → `7e3e5f0` on 2026-07-30 with merge-SHA CI green on all three jobs. Delivered scope: the four Work-surface mutations and manual task creation converged onto the deployed Phase 2E contracts, so `public.tasks` has exactly one validated write path in both the application and the database; `authenticated`'s `insert/update/delete` on `public.tasks` revoked and `update/delete` on `public.reminders` with it, leaving one documented Option C authoring exception; the test suite's meaning migrated statement by statement rather than mechanically re-roled; the command-funnel reader and the end-to-end match baseline published, making ADR-055's two-tier semantic-retrieval evidence gate computable with a dated expiry of `2026-10-27`; and the closeout — a fail-closed traceability generator over all 68 requirements, a phase-wide cleanup verifier, the executed census stop-gate, and the whole-phase convergence audit. Two migrations in the whole phase (`202607290062`, `202607300063`); remote parity `202607300063`. See `docs/reports/phase-2f/PHASE_2F_REPORT.md`.

**This section's original scope is displaced, not deleted (ADR-056).** Phase 2F is defined by the approved `docs/initiatives/phase-2f/PHASE_2F_PRD.md` (**Revision 4.3**; 4.2 corrected three facts found by Slice 2F.4's pre-code inventory and 4.3 corrected three §10 gate-ledger cells found by the closeout sweep — neither changed a requirement): task-domain write convergence onto the deployed Phase 2E contracts, the scoped reminder-authoring exception, task grant revocation, measured matching with the semantic-retrieval evidence gate, and closeout. The scope this section previously named — occurrence/planned/due/completion/reminder date semantics, historical timeline placement, review invalidation, recurrence, and the pagination/responsive/localization finish — is unscheduled future work, gated on repairing review generation first (`docs/TODO.md` M11), with the continuous items (responsive, accessible, localized) remaining per-slice Definition-of-Done obligations rather than a phase.
