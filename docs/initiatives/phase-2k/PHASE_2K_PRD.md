...
- **2K-AUDIT-002:** The three unmeasured items named in audit §8 are **measured** before the slices that depend on them: task-command confirmation expiry semantics (read `202607260059` in full), the copy a zero-source answer currently produces, and whether any surface already renders `occurredAt`. **Executed 2026-08-08 in slice 2K.0.** Two closed by measurement; the third is closed for its *structural* half and recorded **NOT PROVED** for its provider-prose half, which needs an OpenAI credential this slice is not authorized to spend — see `PHASE_2K_SLICE_00_ACCEPTANCE.md`.
- **2K-AUDIT-003:** Hosted parity is re-proved at the start of implementation, not assumed from this document. **Executed 2026-08-08:** `migration list --linked` returned local = remote = `202608080087`, 87 migrations.
...
- **2K-ACT-005:** A confirmation that no longer matches current state resolves to `expired` and requires a fresh confirmation. It is never silently recomputed and applied. **Measured in 2K.0 (OD-2K-5):** `expired` is a **presentation** state covering three *fact-based* database refusals — `55P03` (the object moved), `2E_CONFIRMATION_REQUIRED` (absent, already consumed, or otherwise inapplicable) and `2E_IDEMPOTENCY_MISMATCH` (a divergent payload under the same key). There is no time-based expiry, and this requirement must **not** add one, must **not** create a TTL, and must **not** widen `TASK_COMMAND_OUTCOMES`, which has no `expired` member by ADR-047. **A confirmation is never reused after a return** — see `2K-CONT-005`.
...
- **2K-CONT-003:** The continuity payload is a **closed schema of identifiers only** — exactly enough to locate the conversation, locate the message, locate the object, and restore the visual position. It carries **none** of: a confirmation id, an operation key, `issuedAt`, `observedBefore`, a fingerprint, a patch, a computed preview, a mutation payload, an authorization, or a reusable confirmation. A guard asserts each of those absences by name, and fails against a planted instance of each.
- **2K-CONT-004:** Returning restores the conversation position and the card's visual reference.
- **2K-CONT-005:** Returning **re-derives** the preview server-side **with a new `issuedAt`**, and re-checks authority, ownership, fingerprint and current state from scratch. No stored computed preview is ever restored, and no clock is carried across the navigation. A new derivation therefore produces a **new fingerprint by design** — that is the intended outcome, not a fault, and it is precisely what makes the earlier confirmation unusable.
- **2K-CONT-006:** **Returning always re-derives with a new `issuedAt`, and always requires a fresh confirmation.** There is no branch in which a prior confirmation is reused. If the object changed *in a way provable from the authorized identifiers alone*, the card shows the `expired` presentation state, shows **only** the difference that can be safely re-derived, and requires a new confirmation. If the object did not change, the card shows the freshly derived preview and requires a new confirmation — presented as normal, **not** as an error, and **never** claiming the earlier confirmation is still valid. If a safe difference cannot be reconstructed without persisting forbidden state, the surface **does not invent one**: it says generically that the earlier preview no longer applies, shows the new preview, and requires a new confirmation.
...
- **2K-SRC-004:** Each source shows its **freshness** from the `occurred_at` already retrieved and currently discarded. **Measured in 2K.0:** `occurred_at` is selected by the RPC, mapped to `ChatSource.occurredAt`, and written into the model prompt as an XML attribute at `openai-provider.ts:312` — and it appears in **no `.tsx` file** and in **no persisted citation**. It reaches the model and never the user.
- **2K-SRC-005:** An answer with no qualifying personal evidence **says so explicitly** and is visually distinct from an evidenced answer. **Measured in 2K.0:** today the product has **no structural representation of insufficiency at all** — `chatAnswerSchema` is `{ answer, citedSourceIds }` with no insufficiency field, the prompt sends the literal string `None` for an empty source set, and the only disclosure is one sentence asking the model to "say that plainly" in prose. The signal must therefore be derived from the **retrieval result**, not from `citations.length === 0`, because a turn *with* sources whose model cited none produces the identical empty array.
...
**OD-2K-5 is CLOSED BY MEASUREMENT** in slice 2K.0 (2026-08-08), not by preference. Evidence: `202607260059` read in full, plus ADR-047.

**Answer: no. A confirmation never expires on time. It becomes inapplicable on *facts*, through three distinct refusals.**

- `task_command_confirmations` carries **no `expires_at`, no TTL and no age check** — the columns are `id, user_id, task_id, action, operation_key, request_fingerprint, status ('issued'|'consumed'), created_at, consumed_at`. The absence is deliberate and was recorded as **ADR-047**.
- **The object moved** → `apply_task_command`'s twelve-column staleness gate raises **`55P03`**.
- **Already spent** → the consumption `UPDATE` is guarded on `user_id`, `operation_key`, `status = 'issued'`, `request_fingerprint`, `task_id` and `action`; anything other than exactly one row raises **`P0001` / `2E_CONFIRMATION_REQUIRED`**.
- **The payload changed under the same key** → **`P0001` / `2E_IDEMPOTENCY_MISMATCH`**. Re-binding was explicitly rejected: *"A changed proposal is a new request; it carries a new key."*
- **`issuedAt` is a fingerprint input via `observedBefore`**, so a derivation minted with a new clock produces a different fingerprint and a prior confirmation can no longer be consumed. **This is the mechanism the continuity contract deliberately relies on, not one it works around.** `2K-CONT-005` requires a new `issuedAt` on every return, so the earlier confirmation is unusable *by construction* and a fresh one is always required. Carrying the original clock across a navigation to keep the old confirmation consumable was considered and is **forbidden**: it would make the continuity payload part of a reusable authorization, contradicting OD-2K-D and `2K-CONT-003`.
- **A failed apply does not burn the confirmation**: consumption is inside the mutating transaction, so any later raise rolls it back and the retry finds the row still `issued`.
- **Vocabulary constraint that follows.** `TASK_COMMAND_OUTCOMES` has **no `expired` member** and ADR-047 refuses to add one. Phase 2K's card state `expired` is therefore a **card-level** state that maps onto `rejected_stale` for task commands; it must **not** widen the closed outcome vocabulary.
