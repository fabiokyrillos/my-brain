...
- **2K-ACT-005:** A confirmation that no longer matches current state resolves to `expired` and requires a fresh confirmation. It is never silently recomputed and applied. **Measured in 2K.0 (OD-2K-5):** the card state `expired` is a presentation state covering three *fact-based* database refusals — `55P03` (object moved), `2E_CONFIRMATION_REQUIRED` (already spent or never issued) and `2E_IDEMPOTENCY_MISMATCH` (payload changed under the same key). There is no time-based expiry, and this requirement must **not** add one or widen `TASK_COMMAND_OUTCOMES`, which has no `expired` member by ADR-047.
...
- **2K-CONT-006:** If the object changed, the prior confirmation is marked `expired`, the relevant difference is shown where that can be done without improper content exposure, and a fresh confirmation is required. **Measured constraint from 2K.0:** `issuedAt` is a fingerprint input through `observedBefore`, so a re-derivation minted with a *fresh* clock produces a different fingerprint and the prior confirmation can no longer be consumed. Continuity must carry the **original pinned `issuedAt`** — the property `session.ts` already provides — or must state plainly that a fresh confirmation is required. It must never mint a new clock silently and then report the resulting refusal as a fault.
...
- **`issuedAt` is the hinge.** It is a fingerprint input via `observedBefore`, so **re-deriving with a fresh clock produces a different fingerprint and a prior confirmation can no longer be consumed.** Continuity must therefore carry the original pinned `issuedAt` — which `session.ts` already does — or accept that the user must confirm again.
