...
- `product_events` é um ledger privado de comportamento do funil, separado de auditoria, jobs e custos de IA. Aceita apenas os **39 eventos** e propriedades versionadas em allowlist e nada mais. A contagem cresceu em quatro etapas e cada uma está na cadeia: os **17** originais (`202607170024`); **+2** na migration `202607210034` (`candidate_edit_started`/`candidate_edit_reset`, mais as contagens opcionais `editedCandidateCount`/`editedFieldCount` de `task_candidates_confirmed`, com bound cruzado validado em `private.require_task_candidates_confirmed_edit_counts`); **+3** na Fase 2D (`question_resolved`, `question_effect_previewed`, `question_reinterpret_applied`, migrations `202607230048`/`202607230049`/`202607230050`); **+4** na Fase 2E Slice 2E.7 (`task_command_previewed`, `task_command_disambiguated`, `task_command_applied`, `task_command_undone`, migration `202607280061`); e **+1** na Fase 2H fatia 2H.3 (`rate_limit_refused`, migration `202608070081` — evento próprio e não um `failureKind` dobrado sobre um evento existente, porque nenhum evento existente cobre "uma operação foi recusada antes de começar": `capture_save_failed` fala de um salvamento **tentado**, e uma recusa por ritmo significa que nada foi tentado. O literal `rate_limited` é deliberadamente distinto de `quota` (SH.6), do throttle de auth (SH.5), do ciclo de vida, do CAPTCHA e das falhas de provedor — quem lê o funil precisa poder dizer **qual** controle recusou); **+3** na Fase 2J fatia 2J.7 (`capture_mode_selected`, `voice_transcription_finished`, `attention_item_resolved`, migration `202608080086`); e **+3** na Fase 2K fatia 2K.8 (`conversation_answer_shown`, `conversation_memory_resolved`, `conversation_suggestion_shown`, migration `202608090088` — a **única** migration que a Fase 2K estava autorizada a gastar, destinada exatamente a isto por OD-2K-C. Toda propriedade é enum fechado ou booleano: não existe chave capaz de guardar uma pergunta, uma resposta, um trecho de fonte, ou o nome de uma pessoa ou de um projeto); e **+6** na Fase 2M fatia 2M.1 (`calendar_viewed`, `day_planned`, `day_review_opened`, `day_review_action_applied`, `notification_consent_changed`, `notification_suppressed`, migration `202608110090` — a **primeira** das **duas** migrations que ADR-105 alocou, **não transferíveis**, gasta **antes de qualquer producer existir** por `2M-METRICS-001`. A mesma migration amplia `product_events_surface_check` para admitir a superfície `calendar` que OD-2M-2 assinou. Seis nomes respondem quatro perguntas declaradas antes deles, e um único leitor as faz: `scripts/phase-2m-daily-cycle-funnel.mjs`. Toda propriedade é enum fechado ou uma contagem limitada, e **não existe chave capaz de guardar uma data ou um horário escolhido pelo usuário** — que é exatamente a forma que uma fase de calendário buscaria primeiro e a que `2M-METRICS-004` nomeia explicitamente). Usa `user_id` + `idempotency_key` para deduplicação e não admite captura original, resumo, títulos, respostas, evidências, prompts, conteúdo de arquivos ou erros brutos. *(A contagem dizia "19" até o encerramento da Fase 2F — correta antes da Fase 2D e obsoleta desde então; `src/lib/closeout/phase-2f-documentation.test.ts` agora extrai a lista do CHECK da cadeia e compara, para que a próxima ampliação reprove o build em vez de envelhecer o texto.)*
