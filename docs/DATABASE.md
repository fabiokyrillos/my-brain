...
- `product_events` é um ledger privado de comportamento do funil, separado de auditoria, jobs e custos de IA. Aceita apenas os **26 eventos** e propriedades versionadas em allowlist e nada mais. A contagem cresceu em três etapas e cada uma está na cadeia: os **17** originais (`202607170024`); **+2** na migration `202607210034` (`candidate_edit_started`/`candidate_edit_reset`, mais as contagens opcionais `editedCandidateCount`/`editedFieldCount` de `task_candidates_confirmed`, com bound cruzado validado em `private.require_task_candidates_confirmed_edit_counts`); **+3** na Fase 2D (`question_resolved`, `question_effect_previewed`, `question_reinterpret_applied`, migrations `202607230048`/`202607230049`/`202607230050`); e **+4** na Fase 2E Slice 2E.7 (`task_command_previewed`, `task_command_disambiguated`, `task_command_applied`, `task_command_undone`, migration `202607280061`). Usa `user_id` + `idempotency_key` para deduplicação e não admite captura original, resumo, títulos, respostas, evidências, prompts, conteúdo de arquivos ou erros brutos. *(A contagem dizia "19" até o encerramento da Fase 2F — correta antes da Fase 2D e obsoleta desde então; `src/lib/closeout/phase-2f-documentation.test.ts` agora extrai a lista do CHECK da cadeia e compara, para que a próxima ampliação reprove o build em vez de envelhecer o texto.)*
