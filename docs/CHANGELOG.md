...
Implemented on branch `codex/phase-2f-slice-4` under `docs/PHASE_2F_PRD.md` Revision 4.2. **Undeployed.** Exactly one migration; **no file under `src/` changed**, which is itself the verification — since Slice 2F.3 no application module issued the writes this revokes, so what is removed is a permission, not a caller.
