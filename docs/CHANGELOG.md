...
- **`supabase/tests/phase_2e_task_command_matching.sql`** - 40 pgTAP assertions executed against a from-zero database in CI: the catalog signature, `prosecdef`, `proconfig`, grants for `authenticated` and `anon`, the 8-entry normalizer corpus, symmetric cross-owner denial plus a null-caller case, ordering before truncation, overflow detection, fail-closed status handling, and LIKE-metacharacter hints (`%`, `_`, embedded and trailing backslash - the last being the `22025` case, since backslash is LIKE's default escape).
