...
- `supabase/tests/undo_operation_routing.sql` (18 assertions), `supabase/tests/ai_interpretation_bounds.sql` (20), `supabase/tests/rpc_version_retirement.sql` (24) — router posture and registry integrity; the bounds trigger including proof that it imposes no completeness contract and leaves `user_corrected` alone; and the whole RPC inventory as an executable contract, which is now the *only* gate that can catch a grant change (revoking a grant is a zero-diff change in the generated types).
