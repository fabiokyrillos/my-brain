...
| `2K-METRICS-004` | **built** | Both live gates widened together — the table CHECK and `validate_product_event_properties` — with a verification block that asserts they name the **same** vocabulary name-by-name and **refuses to run vacuously**. That is the check that would have caught `202608080087`'s defect a phase earlier |
...
| `2K-METRICS-007` | **partial** | The consumer exists, is RLS-scoped, authenticates as the owner, writes nothing, and distinguishes "not deployed yet" from "a quiet week". **Remainder:** producer → writer → RLS consumer proved **end to end on the deployed project**, which needs the migration applied. **Destination:** the deployment record, §7 |
| `2K-METRICS-008` | **partial** | No hosted probe has run, so there is no residue to prove zero. **Remainder:** the zero-residue proof after the deployment probe. **Destination:** the deployment record, §7 |
