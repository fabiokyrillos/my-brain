...
| **H6** | i18n fragmented; `next-intl` unused; Portuguese-only errors in English flows | **ACCEPTED** | Canonical mechanism, dependency removed, 34 strings fixed (ADR-036) | `docs/ENGINEERING_STANDARDS.md`; `src/features/chat/actions.test.ts` |
...
**Implementation.** Three jobs in one workflow. `worker`: `deno check` on both deployed entrypoints, `deno test supabase/functions/`. `database`: `supabase start` (Studio, imgproxy, mail catcher, log pipeline and edge runtime excluded to stay inside the timeout; `db`, `kong`, `rest`, `meta` and Auth kept) → `supabase db reset` → `supabase test db --local supabase/tests` → `supabase db lint --local --level warning --fail-on error` → local credentials exported to `GITHUB_ENV` *before* the build (`NEXT_PUBLIC_*` are inlined at build time) → `npm run build` → `e2e/foundation.spec.ts` on desktop and Pixel 7. Failure uploads the Playwright report and 200 lines of every Supabase container log; the stack always stops. `supabase/seed.sql` installs pgTAP into `extensions` and sets the database `search_path`, local/CI only.
...
**Investigation.** The current definition is 414 lines at `202607230050:593-1006`; ten migrations `create or replace` it and one `alter`s it. Characterised the full contract: signature, `SECURITY DEFINER`, `search_path = ''`, grants, the shared preamble *in order* (auth `42501` → owner `for update` lock on `undo_operations` → `P0002` → replay short-circuit on `status = 'undone'` → status validation → expiry), the flat `if/return` dispatch over seven `action_type` values, lock order on the only multi-lock paths (v2/v3-with-`reinterpret`: `undo_operations` → `entries` *before* the questions UPDATE → `jobs`; correction: `undo_operations` → `entries`), all 15 raises with their SQLSTATEs, the absence of any shared epilogue (each branch writes its own `undo_operations` update, its own branch-specific `audit_logs` row, and its own asymmetric return shape), and the unknown-key fall-through (raises `P0001` `Unsupported undo operation` via an inverted guard).

**Coverage matrix.** Exactly seven `action_type` values can be recorded by any shipped RPC, and all seven were dispatched — zero orphans in either direction. The wrinkle that mattered: `confirm_entry_task_candidates_v2`/`_v3`/`_v4` all record the **unversioned** `'confirm_entry_task_candidates'`, and only `_v5`/`_v6` record versioned strings and get the `after_state -> 'resolutions'` integrity gate. Applying that gate to the unversioned key would have broken v2–v4 undo.
...
**Parity result.** 79-case corpus, first run, zero disagreements — including every datetime edge (no timezone, space separator, month 13, hour 24, `2026-02-30`, a real leap day, fractional seconds, numeric input) and the `.trim()`-before-length-check semantics. The validator deliberately ignores unknown properties, mirroring the Zod source of truth (the provider's own JSON Schema already sets `additionalProperties: false`).

**Acceptance.** Accepted. Malformed model output cannot reach domain tables; valid output is unchanged; boundary cases are covered deterministically in both runtimes; the worker tests execute in CI; failure and retry behaviour are preserved.

### D — i18n canonical direction (H6)

**Investigation.** Seven mechanisms, not five: `src/i18n/messages.ts` (3 consumers), typed copy modules with `satisfies` (9 consumers), copy modules with only `as const`, action-local records, component-local records, 77 inline `{ "pt-BR": …, en: … }` literals across 5 files, and 180 `pt ? … : …` ternaries across 36 files, plus 13 duplicate locale-union declarations. `next-intl` had zero imports anywhere. The defect was larger than the review's two files: 34 Portuguese-only strings across four action files, including 13 in `agent/actions.ts` (reminders and upload) that the review did not flag.

**Implementation.** ADR-036. All 34 strings localized. `resolveLocale` (already in `src/lib/preferences.ts`) resolves locale first and independently, which removes the defect class rather than the 34 instances. `next-intl` removed from `package.json` and the lockfile (706 lines of transitive entries). `interpretations/actions.ts`'s three branches use the file's existing `localized()` idiom per the opportunistic-migration rule rather than being rewritten.
...
| `npm test` (Vitest) | **PASS** — 983 tests, 94 files | Baseline was 902/93; net +81 after 3 new files and 2 deletions |
| `npm run build` | **PASS** | See §3.1 |
| `deno check` (both deployed entrypoints) | **PASS** | First static gate this code has ever had |
| `deno test supabase/functions/` | **PASS** — 20 tests | Deno 2.9.4; the pre-existing `dispatch.test.ts` header admitting it had never executed is now false |
| Offline Playwright | **PASS** | See §3.1 |
| Migration reset from zero | **ENVIRONMENT-GATED** | Docker unavailable on this workstation (documented since Phase 2A). First execution is the `database` CI job on this pull request. This is exactly the gate being introduced; it is not claimed as locally verified. |
| Full pgTAP suite | **ENVIRONMENT-GATED** (same reason) | Statically verified instead: all 24 files declare `begin; plan(N); … finish(); rollback;`, and declared plans match counted assertions in all 24 (this check found the `plan(34)` vs 37 drift). Behavioural execution is the CI job. |
| `supabase db lint` | **ENVIRONMENT-GATED** (same reason) | Runs in the `database` job at `--level warning --fail-on error` |
...
### 3.1 Build and Playwright

Recorded in §6 with exact output. Both were run after all source changes were complete.

### 3.2 Why two gates are environment-gated rather than skipped

Docker Desktop is not installed on this workstation — a limitation documented in `CHANGELOG.md` since Phase 2A and in `TODO.md` under external dependencies. The whole point of Workstream A is that these gates should not depend on one machine: they now run on `ubuntu-latest`, where Docker is free. Their first real execution is therefore the pull request's own CI, and the report says so rather than implying local verification. Deno was the mirror case: it was also unavailable, so it was installed (2.9.4) and that gate **was** executed locally.
...
- `npm test` → `Test Files 94 passed (94) / Tests 983 passed (983)`.
- `deno check supabase/functions/process-jobs/index.ts supabase/functions/heartbeat/index.ts` → exit 0.
- `deno test supabase/functions/` → `ok | 20 passed | 0 failed`.
- pgTAP static plan verification → `files with delta != 0: 0 / 24`, `total planned: 821 total counted: 821`.
