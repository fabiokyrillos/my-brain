...
Function, no deploy, no secret, no external integration. **123 requirements
...
ADR-109**, which authorizes no implementation. A **separate timezone initiative
...
`PHASE_2N_TRACEABILITY_CONTRACT.md` (**twenty-six refusals**: nine live during
planning, seventeen armed at implementation). The governing pair lives in
