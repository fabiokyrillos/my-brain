...
Product Projections prework. Data: 2026-07-17. Branch: `codex/phase-2-intelligent-capture`. Histórico preservado no commit `9f0c1e61a0c3211937c750a79d00d22cad2da562` (`feat(projections): introduce product projection contracts`). Este prework é reutilizável pelos slices futuros, mas não conclui o Slice 2X.3 oficial de captura e jobs.
...
Decisão de reconciliação: o escopo foi implementado antes do Slice 2X.3 oficial. Permanece como uma fundação de projeções isolada; os consumidores com queries owner-scoped, a integração visual e o trabalho de captura/jobs continuam fora deste commit e requerem slice posterior.
...
- `docs/STATE.md` — registrou o prework de projeções e a ausência de consumidor de runtime.
- `docs/TODO.md` — registrou o prework de projeções e manteve os próximos slices pendentes de autorização.
- `docs/CHANGELOG.md` — registrou a mudança e as verificações realizadas.
- `docs/reports/phase-2x/PHASE_2X_PROJECTIONS_PREWORK_REPORT.md` — evidência factual do prework, renomeada durante a reconciliação documental.
...
O Slice 2X.4 não foi iniciado por este prework. Qualquer integração futura precisa de autorização explícita e deve preservar esta fronteira: adaptação owner-scoped no servidor antes de entregar DTOs para uma rota ou componente.
