...
2. **The only published match baseline is scope-caveated.** `2E-MATCH-018` was delivered as a **scoring-layer** measurement over a 14-scenario corpus whose `prefilterTier`, `tokenOverlap` and `queryTokenCount` are hand-written (`src/features/task-commands/match-baseline.test.ts`; `docs/reports/phase-2e/PHASE_2E_FINAL_REPORT.md` §3, lines 56–70, and line 145). Its caveat forbids comparing a future end-to-end number against it. `2F-MEASURE-007` discharges the caveat by measuring the same rates **end-to-end against the deployed contract**.
