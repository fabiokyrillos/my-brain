...
5. User-facing copy uses the internationalization system. Do not scatter locale ternaries through components.
   - **The canonical mechanism is a typed feature copy module** in the `src/features/daily-cycle/copy.ts` shape: an explicit `type XCopy = { … }` naming every key, a `const xCopy = { "pt-BR": {…}, en: {…} } satisfies Record<Locale, XCopy>` literal, and a `getXCopy(locale)` accessor. `satisfies` makes a missing key *or* a missing locale a compile error, which no runtime catalog lookup can do. `Locale`, `locales`, `isLocale` and `resolveLocale` come from `src/lib/preferences.ts` — do not redeclare the locale union. See ADR-036.
   - **Server Actions resolve the locale first, independently, before validating anything else** — `const copy = xCopy[resolveLocale(formData.get("locale"))];`. Validation failures are user-facing too, and they happen before the input schema succeeds; deriving copy from the parsed result is what produced Portuguese-only errors in the English product across four action files. Locale reaches an action as a hidden form field; there is no request-scoped locale in this architecture.
   - **New code uses only this mechanism. No new localization mechanism may be introduced.** Legacy mechanisms (`src/i18n/messages.ts`, `localized(locale, pt, en)` helpers, inline `{ "pt-BR": …, en: … }` literals, `const pt = locale === "pt-BR"` ternaries) still exist and are migrated *opportunistically when a file is substantially touched* — not in a big-bang pass. `next-intl` was removed: it was a declared dependency with zero imports.
