...
**Revision 4.1 (2026-07-29). Status: approved and in implementation.** Revision 4 is the governing document; **4.1 is a single narrowly-scoped correction to 2F-CREATE-002**, forced by an objective repository contradiction found during Slice 2F.3's mandatory pre-code inventory and authorized by the owner. It adds no epic, no slice, no feature surface and no architecture. See §6.5a and §14.
...
- **2F-REVOKE-003:** The re-grant rollback script is committed, and CI's `database` job gains a **new harness step, built by this slice** (none exists today — the job currently runs `db reset`, pgTAP, `db lint` and one Playwright spec): apply the revocation, apply the re-grant, and prove by pgTAP that the previously-revoked writes function again. **What this proves:** the re-grant SQL applies and restores the privileges, on a stack whose starting privileges reproduce Supabase's platform defaults — which is also where those privileges originate, since no explicit `grant` on `tasks`/`reminders` exists anywhere in the migration chain (the repository narrows by `revoke`). **What this explicitly does not prove:** an operational rollback of a live database — PostgREST schema-cache convergence and in-flight session behaviour are named residuals, verified instead in the 2F.4 deployment session itself. The claim "rehearsed in CI" may only ever be made with this scope attached.
- **2F-REVOKE-004:** pgTAP proves the denial **non-vacuously**: the same run first asserts the **pre-revocation** privilege state (`authenticated` *can* insert/update/delete on the pre-revocation chain — proving the local stack granted the platform defaults at all), then applies the revocation and proves `authenticated` cannot; `select` and RLS-scoped reads are proven preserved. On `reminders`, it proves exactly the posture the 2F-REMINDER-003 determination produced. The evidence loss is stated, not implied: **write-side RLS on `tasks` becomes untestable from a client role** (the grant refuses before RLS is consulted); the compensating evidence is read-side RLS (a cross-owner `select` returning zero rows) plus RPC-boundary denial (`apply_task_command` against another owner's task raising `P0002`).
...
The Gate 2 inventory found **11 direct writes under `set local role authenticated`** in pgTAP plus 4 affected client-role smoke writes. Each is individually dispositioned in §9's table, which is normative; the requirements below bind the work.

- **2F-TESTMIG-001:** Every one of the 11 pgTAP statements receives the §9 disposition — none is mechanically re-roled, none silently deleted, and each changed test's report entry states exactly what invariant it proves after revocation.
...
- **2F-TESTMIG-005:** The reminder stagings split by grant outcome: the three INSERTs (`apply.sql:2587`, `creation.sql:1158`, `:1179`) remain valid under the Option C posture and are kept as-is, doubling as living proof the retained INSERT grant works; the two UPDATEs (`creation.sql:1115`, `:1135`) follow the 2F-REMINDER-003 determination — restaged as `postgres` if UPDATE is revoked, unchanged otherwise — while keeping their invariant: creation-undo copes with a reminder that moved, or that sits in the dormant `snoozed` state, regardless of who moved it.
...
**2F.4:** Revocation migration applied alone in its session with: the CI re-grant harness (built by this slice) already green, its scope stated per 2F-REVOKE-003; pre-revocation privilege assertion plus denial proofs green (2F-REVOKE-004); the reminders UPDATE/DELETE determination written and executed (or its retention recorded); all 11 pgTAP dispositions landed per §9 with CI green; the four smoke reworks executed; the full remote suite green in-session; `SECURITY.md`/`STATE.md` posture updated; `tasks` allowlist empty.
...
| 8 | `creation.sql:1115` | reminders · update | **Conditional on 2F-REMINDER-003** — restage `postgres` iff UPDATE revoked | Creation-undo copes with a reminder whose `remind_at` moved after creation, whoever moved it |
| 9 | `creation.sql:1135` | reminders · update | **Conditional on 2F-REMINDER-003** — restage `postgres` iff UPDATE revoked | Creation-undo copes with a reminder in the dormant `snoozed` state (keeps the handlers' snoozed branches falsifiable per 2F-REMINDER-004) |
| 10 | `creation.sql:1158` | reminders · insert | **Unaffected under Option C** — keep as-is | Retained INSERT grant; undo behaviour with an extra scheduled reminder present |
| 11 | `creation.sql:1179` | reminders · insert | **Unaffected under Option C** — keep as-is | Retained INSERT grant; undo behaviour with an extra snoozed reminder present |
...
No grant widens anywhere in this phase. **Privilege provenance, stated because two gates turn on it:** no explicit `grant` on `tasks`/`reminders` exists anywhere in the migration chain — the current privileges originate in Supabase's platform default privileges, and the repository narrows by `revoke`; §12's end state is achieved by revoking three of the four default privileges and leaving the rest in place (this is why 2F-REVOKE-004 asserts the starting state before asserting the denial).
