...
- [x] Fast-follow (Issue #3): extend `task_candidates_confirmed` with `editedCandidateCount`/`editedFieldCount` and add the `candidate_edit_started`/`candidate_edit_reset` product events (PRD §14, implementation-plan Task 5), content-free and fail-open. `candidate_edit_started` fires once per entry/candidate/tab-session only from a real field change (title/description/dueAt edit or explicit clear) — never from expand/collapse, rerender, or React Strict Mode remount; `candidate_edit_reset` fires only from the explicit "Restaurar sugestão"/"Reset to suggestion" action (never from "Remover descrição"/"Remover prazo" clear) with `editedFieldCount` computed from the canonical normalized edit payload, not raw touched-field state. `editedCandidateCount`/`editedFieldCount` on `task_candidates_confirmed` are derived server-side in `confirmEntryTasks` from the same validated canonical edits array sent to the RPC (a candidate counts as edited only if its canonical `changes` object is non-empty). No RPC, migration, or database change. 594/594 tests, 0 lint/typecheck errors, production build green, on branch `codex/phase-2c-editable-candidate-analytics` (not pushed). **Residual gap, out of scope for Issue #3 by its own explicit exclusion:** the `public.product_events` table's `event_name` CHECK constraint and `private.record_product_event`'s allowlist (migration `202607170024`) do not yet include `candidate_edit_started`/`candidate_edit_reset`, and `validate_product_event_properties` still only allows `candidateCount` for `task_candidates_confirmed`. Until a follow-up migration adds these, all three analytics calls fail the DB-side allowlist and are dropped fail-open (`recordProductEvent` returns `invalid_payload`, no crash, no product-flow impact) — tracked below.
- [ ] Migration follow-up: add `candidate_edit_started`/`candidate_edit_reset` to the `product_events.event_name` CHECK constraint and to `private.record_product_event`'s allowlist, and extend `validate_product_event_properties`'s `task_candidates_confirmed` case to accept `editedCandidateCount`/`editedFieldCount` (bounded integers), so the Issue #3 analytics events implemented in `src/features/tasks`/`src/features/product-analytics` actually persist instead of being silently dropped at the database boundary.
