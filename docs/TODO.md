...
- [ ] Fast-follow: extend `task_candidates_confirmed` with `editedCandidateCount`/`editedFieldCount` and add the `candidate_edit_started`/`candidate_edit_reset` product events (PRD §14, implementation-plan Task 5), content-free and fail-open, before or shortly after Slice 2C.1 merges.
