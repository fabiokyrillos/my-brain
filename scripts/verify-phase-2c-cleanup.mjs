...
// Fail-closed residual-data check for the Phase 2C aggregate remote gate. It asserts
// that no disposable user, orphaned user-owned row, or remote-smoke storage object
// remains in the linked project after the focused Phase 2C smokes run. The prefix and
// table lists are a superset of every fixture the aggregate touches (Phase 2C
// confirmation plus the shared product-events/daily-cycle/baseline smokes it re-runs).
...
// Phase 2C materializes tasks and their owned relations, dependencies, and candidate
// resolutions, so those tables are scanned for owner-orphaned rows alongside the base set.
const ownedTables = [
  "entries",
  "jobs",
  "attachments",
...
if (disposableUsers.length > 0 || Object.values(orphanCounts).some(Boolean) || remoteSmokeObjects > 0) {
  throw new Error("Disposable Phase 2C smoke data remains in the linked project");
}

console.log("Phase 2C cleanup verification passed:", result);
