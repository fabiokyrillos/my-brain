// Deno-runtime copy of src/features/interpretations/trust-builders.ts.
// The Node original imports only a type (ElementTrustDecision) from
// ./schema, which is structurally identical to TrustDecision already
// defined in trust-policy.ts (score/policy/signals/overrides/evidence).
// Using TrustDecision directly here avoids pulling the zod-based schema
// module (and its extraction-schema dependency) into the Edge Function
// bundle; the algorithm itself is otherwise byte-for-byte identical to
// the Node source. Keep both copies in sync — see docs/DECISIONS.md
// ADR-021.
...
type ElementTrust = Record<"summary" | "concepts" | "occurredAt" | "extractedDates" | "entities", TrustDecision>;
