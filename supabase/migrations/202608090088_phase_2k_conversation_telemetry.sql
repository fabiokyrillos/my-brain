-- Phase 2K, slice 2K.8 -- the Conversar telemetry vocabulary, and its consumer.
--
-- THE ONE MIGRATION THIS PHASE IS AUTHORIZED TO SPEND
--
-- OD-2K-C set a ceiling of one, destined for exactly this. ADR-101 authorizes
-- creating, merging and deploying it. The ceiling was never an obligation --
-- `1 allocated · 0 spent` would have been a legitimate close -- but telemetry
-- cannot be delivered honestly without it: `product_events` constrains
-- `event_name` with a database CHECK and constrains each event's PROPERTY KEYS
-- with `private.validate_product_event_properties`, so there is no application
-- path that can record an event the database does not name.
--
-- That is also the privacy mechanism. `2K-METRICS-002` is enforced by the
-- payload having nowhere to put a question, an answer, a source excerpt, a
-- person's name or a project's name -- not by writers promising not to send one.
--
-- WHY THREE EVENTS
--
-- `2K-METRICS-007` requires a CONSUMER before the phase closes, because SH.6
-- shipped a producer with none and its quota refusals recorded nothing for
-- weeks while the code read as though they did. These three are exactly what
-- `scripts/phase-2k-conversation-funnel-reader.mjs` asks questions of. The
-- larger set the PRD sketched would have declared names nothing would ever read.
--
...
-- `202608080086`). The diff against `202608080086` is exactly the lines these
-- three events add -- and this file was ASSEMBLED from that migration's own
-- text rather than retyped, because a hand-copy of 280 lines is how a
-- pre-existing arm gets silently dropped.
--
-- WHY BOTH GATES, AUDITED TOGETHER
--
-- `2K-METRICS-004`. Enforcement is exactly two gates since `202608080087`
-- deleted the third, frozen copy that had been silently rejecting
-- `rate_limit_refused` since `202608070081`. This migration widens both, and
-- its verification blocks assert both -- including that the two agree
-- name-by-name, which is the check that would have caught that defect.
...
alter table public.product_events
  add constraint product_events_event_name_check check (event_name in (
    ...
    -- Phase 2K slice 2K.8. Three, and the count is the point once more:
    -- `2K-METRICS-007` requires a CONSUMER before close, and these are
    -- exactly what `scripts/phase-2k-conversation-funnel-reader.mjs` asks
    -- questions of. A fourth name nothing reads would be SH.6's failure
...
begin
  ...
  if position('conversation_answer_shown' in definition) = 0
  ...
  if position('rate_limit_refused' in definition) = 0
     ...
     or position('voice_transcription_finished' in definition) = 0 then
...
begin
  ...
  if position('conversation_answer_shown' in body) = 0
     or position('conversation_memory_resolved' in body) = 0
     or position('conversation_suggestion_shown' in body) = 0 then
    raise exception 'the property validator does not know the three Phase 2K events';
  end if;

  if position('task_command_undone' in body) = 0
     or position('attention_item_resolved' in body) = 0
     or position('rate_limit_refused' in body) = 0 then
    raise exception 'the property validator lost a pre-existing event during re-declaration';
  end if;
end $$;

-- `2K-METRICS-004`, and the check that would have caught `202608080087`'s
-- defect a phase earlier: the two live gates must name the SAME vocabulary.
--
...
begin
  ...
  if declared is null or array_length(declared, 1) < 30 then
