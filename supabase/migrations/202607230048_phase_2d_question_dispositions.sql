...
-- (resolve_pending_question_vN, ADR-033) with resolve_pending_question_v2,
-- whose closed discriminated p_resolution accepts the Slice 2D.1 answer kind
-- plus exactly three approved non-answer resolutions:
--
...
--   { "kind": "not_relevant" }
--
-- Per ADR-033 decision 3 (PRD product decision 3), not_relevant reuses the
-- existing 'dismissed' pending_questions.status value and is recorded as a
-- distinct resolution kind on the append-only audit/undo evidence — no
-- status CHECK migration, truthful history preserved.
--
-- Deterministic snooze reactivation (plan default for Slice 2D.2): a
-- 'snoozed' question whose snoozed_until has been reached is treated as
-- 'open' at read time. This migration widens list_needs_attention's
-- open-question predicates accordingly and lets the v2 RPC resolve such a
-- reactivated question (materializing the automatic snoozed -> open
-- transition as part of the resolution's own before-state evidence). No new
-- cron, sweep, worker, or scheduler is introduced.
--
-- Every Slice 2D.1 guarantee carries over unchanged: SECURITY DEFINER,
-- empty search_path, qualified references, no dynamic SQL, execute granted
-- to authenticated only; auth.uid() identity with non-disclosing cross-owner
-- denial (P0002); stale safety under an owner entry lock (55P03);
-- single-winner concurrency (loser observes 55000); canonical SHA-256
-- replay fingerprint on the reserved undo operation (replay returns the
-- original result; same key + different payload raises
-- P0001 / 2D_IDEMPOTENCY_MISMATCH); atomic state + audit + undo; and the
-- immutable interpretation JSON is never touched.
--
-- resolve_pending_question_v1 remains callable and byte-identical
-- (backward compatibility during rollout); consumers cut over to v2 in the
-- application commit. Operation keys are namespaced 'resolve-v2:' so v1 and
-- v2 reservations can never collide.
--
-- Only open -> answered|snoozed|dismissed(:dismissed|:not_relevant) and the
-- automatic snoozed -> open reactivation are permitted; a still-snoozed
-- (future snoozed_until) or terminal question rejects with 55000 — direct
-- terminal-to-terminal transitions are impossible.

create or replace function public.resolve_pending_question_v2(
  p_question_id uuid,
  p_resolution jsonb,
...
begin
  ...
  internal_operation_key := 'resolve-v2:' || normalized_key;
  ...
  case resolution_kind
    when 'answer' then
      allowed_resolution_keys := array['kind', 'answer'];
...
revoke all on function public.resolve_pending_question_v2(uuid, jsonb, text)
  from public, anon;
grant execute on function public.resolve_pending_question_v2(uuid, jsonb, text)
  to authenticated;

-- Extend undo_operation with the resolve_pending_question_v2 restore branch.
-- Reproduced byte-for-byte from migration 202607230046 otherwise (same
-- signature, same grants, every pre-existing branch — including the
-- resolve_pending_question_v1 branch — unchanged). The v2 branch restores
-- the exact prior open state (open, cleared answer/answered_at/
-- snoozed_until) guarded by the status the resolution evidence says it left
-- behind, so undo can never clobber a newer, different resolution of the
-- same question: if the question was meanwhile resolved again (e.g. a
-- deferral elapsed and the reactivated question was answered), the guarded
-- update affects zero rows and the integrity check fails the undo instead
...
begin
  ...
  if operation.action_type = 'resolve_pending_question_v2' then
...
-- Deterministic snooze reactivation for the Needs Attention queue: a
-- 'snoozed' question whose snoozed_until has been reached counts as open
-- again at read time, so the entry deterministically returns to the queue at
-- exactly that instant with no sweep, cron, or worker. Reproduced
-- byte-for-byte from migration 202607220041 otherwise (same signature,
-- security definer, search_path, grants, index reliance); the only change is
-- the widened open-question predicate in the three marked places.
...
-- Product analytics: allow the approved Slice 2D.2 event, question_resolved,
-- whose only property is the bounded resolution kind enum — a privacy-safe
-- granularity that reveals that a non-answer resolution happened, never its
-- content (no question text, answer text, reason, deferral instant, or any
-- free text). Additive extension of the migration 202607170024 contract,
-- following the 202607210034/202607220038/202607220044 precedent: table
-- CHECK constraint, per-event property allowlist, and record_product_event's
-- defense-in-depth name guard; every other branch reproduced byte-for-byte.
