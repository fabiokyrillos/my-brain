-- Forward-fix, discovered while writing this slice's own pgTAP coverage (not
-- assumed from a local read-through): confirm_entry_task_candidates_v4's
-- cross-owner ownership check for personIds/waitingOnPersonIds used
--   candidate_row.value -> 'personIds' || candidate_row.value -> 'waitingOnPersonIds'
-- Postgres gives '->' and '||' the SAME precedence and associates them left
-- to right, so this parsed as
--   (candidate_row.value -> 'personIds' || candidate_row.value) -> 'waitingOnPersonIds'
-- -- concatenating the personIds array with the *entire candidate object* as
-- a single extra element, then applying '->' with a text key to the result
-- (an array), which always returns NULL. jsonb_array_elements_text(NULL) in
-- a FROM clause yields zero rows, so the "not exists" ownership check never
-- found anything to reject: a cross-owner personIds/waitingOnPersonIds value
-- always passed application-level validation. The composite foreign key
-- (task_people_person_owner_fk) still correctly rejected the resulting
-- INSERT with SQLSTATE 23503, so no cross-owner row was ever persisted --
-- this bug degraded the error into a confusing raw FK violation instead of
-- the intended clear '2C_INVALID_RELATION' rejection, not a security defect.
-- project/context ownership checks (single-field, no '||') were unaffected.
--
-- Fixed by parenthesizing both sides of the concatenation. Reproduced
-- byte-for-byte from migration 202607220038 otherwise (same signature,
-- same grants, no other behavior change).
