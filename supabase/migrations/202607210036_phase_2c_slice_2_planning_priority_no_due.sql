...
--
-- Adds a new versioned confirmation RPC (v3) rather than widening v2 in
-- place, per ADR-031's already-approved reasoning (generated-client/PostgREST
-- overload safety, a clear rollback boundary) which the implementation plan
-- carries forward through Slices 2C.2-2C.5. confirm_entry_task_candidates_v2
...
--       "changes": {
--         "title"?: text, "description"?: text|null, "dueAt"?: text|null,
--         "plannedAt"?: text|null, "manualPriority"?: text|null,
--         "intentionalNoDue"?: boolean, "noDueReason"?: text|null
--       }
--     }, ...
--   ]
-- }
-- Selected indices and edit objects are sorted by candidate index. Values
-- equal to the immutable suggestion (always null/false for the four new
-- fields, which the AI never proposes) and empty changes objects are
-- omitted, exactly like v2.

-- Defense-in-depth: enforce due/no-due mutual consistency at the table
-- boundary for every insert path, not just this RPC. Satisfied by every
-- existing row (intentional_no_due defaults false, no_due_reason defaults
-- null), so this is safe to add additively.
alter table public.tasks
  add constraint tasks_no_due_consistency_check
  check (
    (no_due_reason is null or intentional_no_due)
    and (not intentional_no_due or due_at is null)
  );
...
-- Extend the existing correction-race guard (migration 202607190033) to also
-- protect tasks materialized by a v3 confirmation. The function/trigger keep
-- their original "v2" name for continuity; the body now checks both
-- namespaces explicitly (not a generic version-agnostic pattern, since only
-- v2 and v3 exist today).
