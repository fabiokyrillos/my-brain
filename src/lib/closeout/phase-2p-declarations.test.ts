...
describe("Phase 2P declarations", () => {
  ...
  it("admits the closeout artifacts now that slice 2P.8 is the slice being written", () => {
    /*
      Flipped, and this is the flip ADR-122 Decision 2 named in advance.

      Until slice 2P.8 these two files were forbidden and this test asserted
      their absence. 2P.8 is the closeout slice, so the same decision that
      forbade them is what now requires them — and asserting absence here would
      forbid the artifact the phase exists to produce.

      **The posture the decision states is unchanged and still asserted**: the
      sentence naming 2P.8 as the boundary must still be in `DECISIONS.md`, so
      the flip cannot be read as the rule having been dropped.
    */
    for (const file of [
      "docs/reports/phase-2p/PHASE_2P_TRACEABILITY_MATRIX.md",
      "docs/reports/phase-2p/PHASE_2P_SLICE_08_ACCEPTANCE.md",
    ]) expect(exists(file), file).toBe(true);
    expect(read("docs/DECISIONS.md")).toMatch(/remain \*\*forbidden until slice 2P\.8\*\*/);

    // And the matrix is generated rather than typed — the same decision's
    // second clause. `phase-2p-traceability.test.ts` is what enforces it byte
    // for byte; this asserts the marker a hand-written file would not carry.
    expect(read("docs/reports/phase-2p/PHASE_2P_TRACEABILITY_MATRIX.md")).toContain(
      "Do not edit by hand. Generated by scripts/generate-phase-2p-traceability.mjs",
    );
  });

  it("records the four decisions the owner signed at closeout, by amendment", () => {
    /*
      Each is asserted by the clause that would be lost if the amendment were
      softened, not by its date — a date matches a mention of it.
    */
    const decisions = read("docs/DECISIONS.md");
    // `2P-ACCESS-003` — the navigation the product uses, and the five prior
    // decisions explicitly not reversed.
    expect(decisions).toMatch(/the five prior decisions are not reversed/);
    // The month's telemetry — refused, and refusal 9 told not to fire on it.
    expect(decisions).toMatch(/must not fire on this\*\*, because what is absent is an event for one orientation/);
    // The reminder repair — authorized, without a migration, reproduce first.
    expect(decisions).toMatch(/authorizes repairing `2P-REMINDER-REVALIDATE-HANG` inside slice 2P\.8/);
    // The count — 72, corrected forward, and no eighty-eighth requirement.
    expect(decisions).toMatch(/\*\*No eighty-eighth requirement is invented\*\*/);
  });
  ...
  it("spends exactly the two migrations the owner authorized, by name", () => {
    ...
    */
    ...
    // Phase 2R spent the ONE migration ADR-132 Decision 8 allocated, named here
    // rather than absorbed into the total, so a SECOND fails on this line and an
    // unattributed one still fails the count below.
