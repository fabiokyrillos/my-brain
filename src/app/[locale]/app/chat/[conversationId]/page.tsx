...
export default async function ConversationPage({ params }: { params: Promise<{ locale: string; conversationId: string }> }) {
  ...
  return <div className="content-page chat-thread"><Link href={`/${locale}/app/chat`} className="back-link"><ArrowLeft size={16} />{pt ? "Conversas" : "Conversations"}</Link><header><p className="eyebrow">{pt ? "BRAIN COM FONTES" : "BRAIN WITH SOURCES"}</p><h1>{conversation.title}</h1></header><div className="message-stream">{messages.map((message) => { const citations = Array.isArray(message.citations) ? message.citations as Citation[] : []; return <article className={`chat-message ${message.role}`} key={message.id}><span>{message.role === "user" ? (pt ? "Você" : "You") : agentName}</span><p>{message.content}</p>{citations.length > 0 && <div className="message-sources"><strong><BookOpenText size={14} />{pt ? "Fontes internas" : "Internal sources"}</strong>{citations.map((citation) => citation.type === "entry" ? <Link href={`/${locale}/app/inbox/${citation.sourceId}`} key={citation.id}>{citation.excerpt}</Link> : <Link href={`/${locale}/app/memories/${citation.sourceId}`} key={citation.id}>{citation.excerpt}</Link>)}</div>}{message.model && <small>{message.model}</small>}</article>; })}</div><AssistantComposer action={runAssistantTurn} agentName={agentName} conversationId={conversationId} locale={locale} memoryAction={createProposedMemory} memoryUndoAction={undoProposedMemory} /></div>;
}
