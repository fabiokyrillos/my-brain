...
export default async function PeoplePage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<{ page?: string | string[] }> }) {
  ...
  return <div className="content-page"><header className="list-header"><div><p className="eyebrow">{pt ? "RELAÇÕES" : "RELATIONSHIPS"}</p><h1>{pt ? "Pessoas" : "People"}</h1><p>{pt ? "Quem aparece nas suas conversas, decisões e acompanhamentos." : "People who appear in your conversations, decisions, and follow-ups."}</p></div><InlineCreateForm action={createRecord} kind="person" locale={locale} /></header>{items.length ? <div className="list-stack">{items.map((person) => <Link href={`/${locale}/app/people/${person.id}`} className="list-row" key={person.id}><div className="list-row-main"><strong>{person.name}</strong><p>{pt ? "Aparece no seu contexto" : "Appears in your context"}</p></div></Link>)}</div> : <div className="empty-list"><UsersRound size={30} /><strong>{pt ? "Nenhuma pessoa ainda" : "No people yet"}</strong><p>{pt ? "Pessoas citadas nas capturas aparecem aqui automaticamente." : "People mentioned in captures appear here automatically."}</p></div>}<PaginationLinks locale={locale} path="people" page={page} hasNext={hasNext} /></div>;
}
