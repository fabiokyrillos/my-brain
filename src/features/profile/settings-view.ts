...
export async function loadSettingsFormValues(supabase: SupabaseClient, userId: string): Promise<SettingsFormValues> {
  const [profileResult, preferencesResult] = await Promise.all([
    ...
    supabase.from("agent_preferences").select("agent_name,personality,tone,quiet_start,quiet_end,important_reminder_override,max_followups_per_day,response_detail,ai_profile,chat_model,extraction_model,review_model,file_model").eq("user_id", userId).maybeSingle(),
  ]);
