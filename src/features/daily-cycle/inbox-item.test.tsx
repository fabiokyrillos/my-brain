...
describe("InboxItemRow", () => {
  it("renders the human title, original preview, and a localized product-state label", () => {
    render(<InboxItemRow agentName="Brain" item={item()} locale="pt-BR" />);
  ...
  it("never renders the internal product state string as UI text", () => {
    render(<InboxItemRow agentName="Brain" item={item({ productState: "could_not_organize" })} locale="en" />);
  ...
  it("surfaces the attention reason as an accessible hint when one is present", () => {
    render(<InboxItemRow agentName="Brain" item={item({ productState: "needs_attention", attentionReason: "confirm_existing_candidates" })} locale="en" />);
