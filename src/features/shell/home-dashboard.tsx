...
export async function HomeDashboard({ locale }: { locale: Locale }) {
  ...
  const [tasksResult, waitingResult, questionsResult, preferencesResult, inboxProjection, attentionProjection] = await Promise.all([
  ...
  return <div className="dashboard">
    ...
    <section className="dashboard-grid">
      ...
      <article className="panel review-panel"><header><div><span className="panel-kicker">05 / RITMO</span><h2>{t.nextReview}</h2></div></header><div className="review-time"><strong>{reviewTime}</strong><span>{pt ? "Horário preferido" : "Preferred time"}</span></div></article>
