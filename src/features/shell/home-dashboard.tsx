...
export async function HomeDashboard({ locale }: { locale: Locale }) {
  ...
  const [tasksResult, waitingResult, questionsResult, preferencesResult, inboxProjection] = await Promise.all([
  ...
  return <div className="dashboard">
    ...
    <section className="dashboard-grid">
      ...
      <article className="panel recent-panel"><header><div><span className="panel-kicker">05 / RECENTE</span><h2>{t.recent}</h2></div></header>{recentItems.length ? <div className="dashboard-recent-list">{recentItems.map((item) => <InboxItemRow item={item} key={item.entryId} locale={locale} />)}</div> : <p className="quiet-state">{pt ? "Nada por aqui ainda. Capture algo para começar." : "Nothing here yet. Capture something to get started."}</p>}</article>
    </section>
  </div>;
}
