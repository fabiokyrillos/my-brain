...
- `docs/reports/` — the historical record, filed **one directory per phase or initiative** (`phase-2f/`, `byok/`, `signup-hardening/`, …). Start at `docs/reports/README.md`. A new phase creates its directory before its first report; nothing new goes at the reports root, and `src/lib/closeout/reports-taxonomy-guard.test.ts` fails CI if it does. Durable loop handoff state: `docs/reports/AUTONOMOUS_LOOP_HANDOFF.md` (§1–§32) continued by `AUTONOMOUS_LOOP_HANDOFF.md` at the repository root (§33 onward) — read both.
