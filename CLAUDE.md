...
**Capture → interpretation**: a Server Action authenticates and writes `entries.original_content` immutably, then calls `AIProvider.extractEntry` (OpenAI Responses API + Structured Outputs, Zod-validated) for a schema covering concepts, entities, candidate tasks, pending questions, and confidence. A transactional RPC persists the interpretation, entities, event date, and audit row. Embedding generation is separate and non-blocking — an embedding failure never destroys the interpretation. The UI shows interpretation + original + candidate tasks; only user-confirmed tasks are materialized via an idempotent RPC that links people/projects/contexts and records an undo-compensation entry.

**Async job path (in progress)**: `capture_entry_async` persists an entry and a minimal `interpret_entry` job atomically; `enqueue_entry_reprocessing` enqueues reprocessing idempotently without touching the current interpretation. Claims (`claim_entry_interpretation_job`, `claim_next_entry_interpretation_job`) are `service_role`-only, lease-based, and use `FOR UPDATE SKIP LOCKED`. The live UI still runs the synchronous Phase 2B flow — this async contract exists but has no consumer yet (see `docs/STATE.md` for current slice status before assuming otherwise).
...
**Jobs**: `jobs` table with status/attempts/next-attempt/priority/idempotency. Uploads enqueue jobs and invoke the authenticated `process-jobs` Edge Function (signed URL, separate interpretation, lease-based claim, bounded retry/backoff, reaper for expired leases).
...
Treat `docs/STATE.md` as the current source of truth for what phase/slice is active and what's actually shipped vs. planned — don't infer status from code alone, since in-progress contracts (e.g. the async job path above) can exist without a live consumer yet. `docs/TODO.md` tracks the active backlog by priority; `docs/DECISIONS.md` is an append-only ADR log — add to it, don't rewrite history.
