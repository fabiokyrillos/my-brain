...
CI (`.github/workflows/ci.yml`) runs three parallel jobs (ADR-038):
- `app` — `npm ci`, `lint`, `typecheck`, `test`, `build`.
- `worker` — `deno check` on both deployed Edge Function entrypoints, then `deno test` over every directory under `supabase/functions/`, with no `--allow-*` flags so it can never reach a network.
- `database` — a local Supabase stack, `supabase db reset` (the whole migration chain applied from an empty database), the full pgTAP suite, `supabase db lint --schema public,private`, then `e2e/foundation.spec.ts` against the production build on desktop and Pixel 7.

Remote smokes and the authenticated online journeys stay manual. There is no lint/format-on-save autofixer configured — fix reported errors directly.
...
- `docs/` — living documentation; treat as authoritative context, not background reading. Start at `docs/README.md`. The `docs/` root holds only the **living canon**: `STATE.md` (current phase/status, source-of-truth order), `TODO.md` (backlog), `DECISIONS.md` (append-only ADRs), `CHANGELOG.md`, `ENGINEERING_STANDARDS.md` (mandatory engineering contract), `ARCHITECTURE.md`, `DATABASE.md`, `AI_AGENT.md`, `SECURITY.md`, plus the whole-product `PRD.md` and `IMPLEMENTATION_PLAN.md`.
- `docs/initiatives/<name>/` — **governing artifacts** (PRD, implementation plan, proposal, closing report) for one phase or initiative: `phase-2f/`, `byok/`, `signup-hardening/`, `entity-graph/`, `product-ux/`, … `docs/reports/<name>/` — the **record** of that same work, under the same directory name. A new phase creates both before its first file; nothing new goes at either root, and `src/lib/closeout/docs-taxonomy-guard.test.ts` and `reports-taxonomy-guard.test.ts` fail CI if it does.
- Durable loop handoff state: `docs/reports/AUTONOMOUS_LOOP_HANDOFF.md` (§1–§32) continued by `AUTONOMOUS_LOOP_HANDOFF.md` at the repository root (§33 onward) — read both.

### Core flows

**Capture → interpretation (asynchronous since Phase 2X Slice 2X.5)**: the `captureEntry` Server Action authenticates and atomically persists `entries.original_content` plus a minimal `interpret_entry` job via `capture_entry_async`, then returns immediately — no redirect, no synchronous AI call. It builds a `CaptureReceipt` and, inside a `next/server` `after()` callback (so it never adds latency to the response), nudges the deployed worker and records best-effort product events; an unattended `pg_cron`/`pg_net` drain is the correctness backstop if that nudge fails. The worker (`supabase/functions/process-jobs/entry.ts`) reloads the entry and calls the OpenAI Responses API with Structured Outputs for a schema covering concepts, entities, candidate tasks, pending questions, and confidence, then validates the parsed output against `supabase/functions/_shared/extraction-validation.ts` before persisting via a transactional RPC (interpretation, entities, event date, audit row). It does **not** call `AIProvider.extractEntry`: `src/lib/ai` cannot be imported from Deno (the `server-only` guard throws), so the worker has its own implementation of the prompt, the response schema and the validation. `src/lib/ai/extraction-schema.ts` remains the source of truth for the contract, and `src/lib/ai/extraction-parity.test.ts` fails the build if the two ever disagree. Embedding generation is separate and non-blocking — an embedding failure never destroys the interpretation. The UI shows interpretation + original + candidate tasks once the worker finishes; only user-confirmed tasks are materialized via an idempotent RPC that links people/projects/contexts and records an undo-compensation entry. `reprocessEntry` follows the same enqueue-and-return shape via `enqueue_entry_reprocessing`. Claims (`claim_entry_interpretation_job`, `claim_next_entry_interpretation_job`) are `service_role`-only, lease-based, and use `FOR UPDATE SKIP LOCKED`.
...
Treat `docs/STATE.md` as the current source of truth for what phase/slice is active and what's actually shipped vs. planned — don't infer status from code alone, since a contract can exist without a live UI consumer yet. (Both of the examples this line used to give are now resolved: `retryProcessingJob` was removed as consumer-less, and the daily-cycle Inbox/Needs-Attention/Work projection mappers all ship in production — see the pre-2E hardening report.). `docs/TODO.md` tracks the active backlog by priority; `docs/DECISIONS.md` is an append-only ADR log — add to it, don't rewrite history.
